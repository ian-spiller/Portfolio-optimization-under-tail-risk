# run_target_vol.py
# Rolling Target-Volatility allocation
#
...
from src.optimizers import opt_target_vol
...
TICKERS = {
    "SP500": "SPY",
    "UST_7_10Y": "IEF",
    "T_BILLS": "SHY",     # investable cash proxy here
    "CHINA": "FXI",
    "REITS": "VNQ",
    "GOLD": "GLD",
}
...
VOL_CONSTRAINT = "ineq"   # inequality constraint recommended (kept for your reference)
...
OUTPUT_XLSX = RESULTS_DIR / "target_vol_results.xlsx"
...
def main() -> None:
    ...
    # 3) Rolling target-vol optimization
    W, R, meta, diag = rolling_optimize(
        ...
        min_coverage=1.0,
        rf_col=None,
        optimizer=opt_target_vol,
        optimizer_kwargs={"target_vol_annual": TARGET_VOL_ANNUAL},
    )

    print("--------------------------------------------------")
    print(f"Rolling Target Volatility | target = {TARGET_VOL_ANNUAL:.1%} p.a.")
    ...
    # ---- Transaction costs + choose export return series for factor/scenario work
    export_R = R
    if APPLY_T_COSTS:
    ...
    # Scenario simulation (separate try so factor outputs still export)
    if factors is not None and roll is not None:
        try:
            ...
            # Optional sample to keep Excel small
    ...
    export_excel(
        ...
        weights=W,
        oos_returns=export_R,  # this is NET if APPLY_T_COSTS else GROSS
        bounds_meta=meta,
